...
Below is a detailed walkthrough of each function in the `Code.gs` script that powers the simulator.
...
function TradeSimulator() {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const simCountCell  = spreadsheet.getRangeByName("simCount");
  const simsToPerform = simCountCell.getValue();
  const namedRanges   = getNamedRanges(spreadsheet);

  // Run all simulations and collect results
  const results = performSimulations(namedRanges, simsToPerform);
  ...
  // Ensure the equity cell has settled within expected bounds
  refreshEquityValueIsInRange(
    namedRanges.fourHundredTradesEquity,
    namedRanges.minValue,
    namedRanges.maxValue
  );
}
````

1. **Grab settings**: Reads your `simCount` named range to know how many randomized paths to run.
2. **Bundle output cells**: `getNamedRanges()` returns an object mapping each named range (e.g. `expectedValue`, `minValue`, etc.) for easy access.
3. **Simulate**: `performSimulations()` loops through each trial, triggering the sheet’s 450-trade formula and collecting the end equity.
4. **Summarize**: `calculateSummaryStatistics()` and `getMedianQ1Q3()` compute mean, median, min/max, SD, Q1, Q3. Results are written back via `updateSummaryStatistics()`.
5. **Stabilize**: `refreshEquityValueIsInRange()` polls the equity cell until its value falls within the computed min/max (helps with slower recalc).

---

### 2. `getNamedRanges(spreadsheet)`

```js
function getNamedRanges(ss) {
  return {
    expectedValue:         ss.getRangeByName("expectedValue"),
    medianValue:           ss.getRangeByName("medianValue"),
    minValue:              ss.getRangeByName("minValue"),
    maxValue:              ss.getRangeByName("maxValue"),
    standardDeviation:     ss.getRangeByName("standardDeviation"),
    fourHundredTradesEquity: ss.getRangeByName("fourHundredTradesEquity"),
    q1Value:               ss.getRangeByName("q1Value"),
    q3Value:               ss.getRangeByName("q3Value")
  };
}
```

* Returns an object of all the named ranges used for inputs, outputs, and intermediate values.

---

### 3. `performSimulations(namedRanges, count)`

```js
function performSimulations(namedRanges, count) {
  const results = [];
  for (let i = 0; i < count; i++) {
    // Trigger and capture the sheet’s formula result
    const equity = namedRanges.fourHundredTradesEquity.getValue();
    results.push(equity);
...
* **Capture**: Reading `fourHundredTradesEquity` forces the sheet to recalc the 450-trade formula and returns that equity.
* **Progress UI**: `updateInProgress()` writes “Updating…Sim X” to all output cells except the equity cell.

---

### 4. `updateInProgress(namedRanges, currentSimulation)`
...
* Writes a dynamic progress message into each output cell (excluding the equity cell) so you can watch the simulation index in real time.

---

### 5. `calculateSummaryStatistics(results)`
...
function calculateSummaryStatistics(results) {
  ...
  return {
    ...
    maxValue:         Math.max(...results),
    standardDeviation: calculateStandardDeviation(results),
    q1:               sorted[(sorted.length / 4) | 0],
    q3:               sorted[(sorted.length * 3 / 4) | 0]
  };
}
```

* Sorts results for quartile/median picks.
* Computes mean, min, max, and delegates SD to `calculateStandardDeviation()`.

---

### 6. `calculateStandardDeviation(results)`

```js
function calculateStandardDeviation(results) {
  const mean     = results.reduce((a, v) => a + v, 0) / results.length;
  const variance = results
    .reduce((a, v) => a + Math.pow(v - mean, 2), 0)
    / results.length;
  return Math.sqrt(variance);
}
```

* Implements the population SD formula: √(variance), where variance is the average squared deviation from the mean.

---

### 7. `getMedianQ1Q3(namedRanges, results)`

```js
function getMedianQ1Q3(namedRanges, results) {
  const sorted = results.slice().sort((a, b) => a - b);

  namedRanges.medianValue.setValue(sorted[(sorted.length / 2) | 0]);
  namedRanges.q1Value.setValue(sorted[(sorted.length / 4) | 0]);
  namedRanges.q3Value.setValue(sorted[(sorted.length * 3 / 4) | 0]);
}
```

* Writes median, first quartile, and third quartile directly back to their named cells.

---

### 8. `updateSummaryStatistics(namedRanges, summaryStats)`
...
* Iterates your named output ranges (except the equity cell) and sets each to its corresponding value in the `summaryStats` object.

---

### 9. `refreshEquityValueIsInRange(equityCell, minCell, maxCell)`

```js
function refreshEquityValueIsInRange(equityCell, minCell, maxCell) {
  let equity = equityCell.getValue();
  let minVal  = minCell.getValue();
  let maxVal  = maxCell.getValue();

  while (equity < minVal || equity > maxVal) {
    SpreadsheetApp.flush();
    Utilities.sleep(50);
    equity = equityCell.getValue();
    minVal  = minCell.getValue();
    maxVal  = maxCell.getValue();
  }
}
```

* Ensures the 450-trade equity value has finished recalculating and lies within the expected `[minValue, maxValue]` range before proceeding.

---

With these explanations, every part of your Apps Script should be clear—each function’s purpose, how they interconnect, and how they drive the Monte Carlo simulation inside Google Sheets.\`\`\`
